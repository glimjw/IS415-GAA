...
```{r}
cluster_vars.cor = cor(ict_derived[,7:17])
...
shan_sf_ngeo_cluster %>% 
  st_set_geometry(NULL) %>%
  group_by(CLUSTER) %>%
  summarise(mean_RADIO_PR = mean(RADIO_PR.y),
            mean_TV_PR = mean(TV_PR.y),
            mean_LLPHONE_PR = mean(LLPHONE_PR.y),
            mean_MPHONE_PR = mean(MPHONE_PR.y),
            mean_COMPUTER_PR = mean(COMPUTER_PR.y))
```
