...
From the simulation results, we can observe that the p-value is very small, at 0.001, which strongly rejects the null hypothesis of no spatial autocorrelation. This means that there is spatial autocorrelation in the distribution of Number of dengue cases across villages

#### 3.4.3 Visualising Monte Carlo Geary's C 

Let's display our findings in a histogram so that we can visualise it better

Firstly, we can get a statistical summary of the Monte Carlo Geary's C output
...
```{r}
#| echo: false
hist(bperm1$res, freq=TRUE, breaks=20, xlab="Simulated Geary c")
abline(v=1, col="red") 
```
