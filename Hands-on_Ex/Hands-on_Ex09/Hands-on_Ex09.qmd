...
```{r}
write_rds(train_data, "In-class_Ex09/data/model/train_data.rds")
write_rds(test_data, "In-class_Ex09/data/model/test_data.rds")
```
...
```{r}
train_data <- read_rds("In-class_Ex09/data/model/train_data.rds")
test_data <- read_rds("In-class_Ex09/data/model/test_data.rds")
```
...
```{r}
write_rds(price_mlr, "In-class_Ex09/data/model/price_mlr.rds" ) 
```
...
**Note:** The code chunk below is used to determine adaptive bandwidth and CV method is used to determine the optimal bandwidth.
...
```{r}
#| eval: false
write_rds(bw_adaptive, "In-class_Ex09/data/model/bw_adaptive.rds")
```
...
```{r}
bw_adaptive <- read_rds("In-class_Ex09/data/model/bw_adaptive.rds")
```
...
```{r}
#| eval: false
write_rds(gwr_adaptive, "In-class_Ex09/data/model/gwr_adaptive.rds")
```
...
```{r}
gwr_adaptive <- read_rds("In-class_Ex09/data/model/gwr_adaptive.rds")
```
...
```{r}
coords_train <- write_rds(coords_train, "In-class_Ex09/data/model/coords_train.rds" )
coords_test <- write_rds(coords_test, "In-class_Ex09/data/model/coords_test.rds" )
```
...
write_rds(rf, "In-class_Ex09/data/model/rf.rds")
```

```{r}
rf <- read_rds("In-class_Ex09/data/model/rf.rds")
rf
```
...
```{r}
#| eval: false
write_rds(gwRF_adaptive, "In-class_Ex09/data/model/gwRF_adaptive.rds")
```

The code chunk below can be used to retrieve the save model in future.

```{r}
gwRF_adaptive <- read_rds("In-class_Ex09/data/model/gwRF_adaptive.rds")
```
...
```{r}
# Specify the size of the sample you want
sample_size <- 100  # Adjust this value as needed

# Create a random sample of row indices from your dataset
sample_indices <- sample(nrow(test_data), size = sample_size, replace = FALSE)

# Use the sampled row indices to extract the corresponding rows from the dataset
sample_data <- test_data[sample_indices, , drop = FALSE]
...
```{r}
#| eval: false
GRF_pred <- write_rds(gwRF_pred, "In-class_Ex09/data/model/GRF_pred.rds")
```
...
```{r}
GRF_pred <- read_rds("In-class_Ex09/data/model/GRF_pred.rds")
...
```{r}
test_data_p <- cbind(sample_data, GRF_pred_df)
```

```{r}
write_rds(test_data_p, "In-class_Ex09/data/model/test_data_p.rds")
```
